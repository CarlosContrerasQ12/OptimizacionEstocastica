...
def animarSolucion(nombreArch):
	data=np.load("data/"+nombreArch+".npz")
	sol=data["sol"]
	tsol=data["t"]
	...
	anim = FuncAnimation(fig, animate, init_func=init,frames=range(2,200,1), interval=20)
	anim.save(nombreArch+'.mp4')
	plt.close(fig)
#plt.show()
